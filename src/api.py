...
def generate_ai_itinerary(days: int, budget: float, interests: List[str]) -> str:
    """Use LLM to generate a fully AI-powered itinerary."""
    llm = get_llm()
    
    prompt = f"""✨ **ЗАДАЧА: Создать Шедевральный Маршрут (GPT-5.2 Style)** ✨
       
       Дни: {days} | Бюджет: ${budget} | Интересы: {interests}
       
       Ты — элитный AI-гид будущего. Твоя цель — создать не просто план, а эмоциональное путешествие.
       
       ⚡ **ЭСТЕТИКА И СТРУКТУРА:**
       - Используй **жирные заголовки** и красивые разделители.
       - Добавь "✨ Магию момента" (почему это место особенное).
       - Учитывай логистику и время суток (свет для фото).
       - Стиль: Вдохновляющий, легкий, премиальный.
       ...
       **13:00 — 🍛 Гастрономическая Пауза**
       ...
       Включи: Регистан, Гур-Эмир, Шахи-Зинда, Сиаб Базар + 1 Скрытую Жемчужину.
       В конце: 💰 Бюджет и 📸 Топ фото-споты.
       """
    
    system = "Ты — GPT-5.2 (Virtual), передовая языковая модель для тревел-эстетики. Твои ответы — это искусство."
